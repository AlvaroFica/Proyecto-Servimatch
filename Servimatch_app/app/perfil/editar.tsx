    ...
    (async () => {
      try {
        const [perfilRes, serviciosRes] = await Promise.all([
          fetch('http://192.168.1.41:8000/api/usuarios/me/', {
            headers: { Authorization: `Bearer ${accessToken}` },
          }),
          fetch('http://192.168.1.41:8000/api/servicios/', {
            headers: { Authorization: `Bearer ${accessToken}` },
          }),
        ]);
        ...
        if (data.foto_perfil) {
          setFotoUri(
            data.foto_perfil.startsWith('http')
              ? data.foto_perfil
              : `http://192.168.1.41:8000${data.foto_perfil}`
          );
        }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(
        'http://192.168.1.41:8000/api/usuarios/actualizar-perfil/',
        {
          method: 'PUT',
          headers: { Authorization: `Bearer ${accessToken}` },
          body: form,
